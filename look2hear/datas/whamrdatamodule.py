...
class WhamrDataset(Dataset):
    def __init__(
        ...
        mix_json = os.path.join(json_dir, "mix_both_reverb.json")
        sources_json = [
            os.path.join(json_dir, source + ".json") for source in ["s1_reverb", "s2_reverb"]
        ]
